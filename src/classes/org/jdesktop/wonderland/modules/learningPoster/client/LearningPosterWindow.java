...
/**
 *
 * The window for the learningPoster app.
 * @author Bernard Horan
 * @author Jon Kaplan
 *
 */
public class LearningPosterWindow extends WindowSwing {

    /** The cell in which this window is displayed. */
    private final LearningPosterCell cell;

    /** The panel displayed within this window. */
    private LearningPosterPanel learningPosterPanel;

    /** The learningPoster node to render the label */
    private final learningPosterNode learningPosterNode;
     ...
     */
    public LearningPosterWindow(final LearningPosterCell cell, App2D app, int width,
            int height, boolean decorated, Vector2f pixelScale)
            throws InstantiationException {
        super(app, width, height, decorated, pixelScale);
        this.cell = cell;

        setTitle("learningPoster");
        View2D view = getView(cell);
        learningPosterNode = new learningPosterNode(view);
        view.setGeometryNode(learningPosterNode);

        try {
            SwingUtilities.invokeAndWait(new Runnable () {
                public void run () {
                    // This must be invoked on the AWT Event Dispatch Thread
                    learningPosterPanel = new LearningPosterPanel(cell);
                }
            });
        } catch (Exception ex) {
        ...
        JmeClientMain.getFrame().getCanvas3DPanel().add(learningPosterPanel);

        setComponent(learningPosterPanel);
    }
    ...
    void updateLabel() {
        learningPosterNode.setBillboard(cell.getBillboardMode());
        learningPosterPanel.updateLabel();
    }

    /**
     * Listener that passes fake mouse events to the learningPoster pane. This
     * allows the learningPoster to react to mouseovers and clicks even though
     * we don't have control of the learningPoster
     */
    private static class FakeMouseEventListener extends EventClassListener {
        private final LearningPosterWindow window;
        private final View2D view;
        
        public FakeMouseEventListener(LearningPosterWindow window, View2D view) {
            this.window = window;
            this.view = view;
}
        ...
        public void commitEvent(Event event) {
            ...
            final MouseEvent fake = new MouseEvent(
                    window.learningPosterPanel.getLearningPosterPane(),
            ...
            SwingUtilities.invokeLater(new Runnable() {
                public void run() {
                    window.learningPosterPanel.getLearningPosterPane().dispatchEvent(fake);
                }
            });
        }
    }
